...
from analysis import to_df, filter_by_artist, kpi, newest_tracks
from plots import (
    plot_duration_hist,
    plot_year_counts,
    plot_top_genres,
...
st.sidebar.title("Wyszukaj artystę")
search_section = st.sidebar.container()
with search_section:
    artist = st.text_input("Artysta", "Metallica")
    run_analysis = st.button("Pobierz i analizuj", use_container_width=True)

st.sidebar.divider()

st.sidebar.markdown("### Projekt")
show_docs = st.sidebar.button("Dokumentacja projektu", use_container_width=True)
...
if show_docs:
    ...
    try:
        ...
        st.warning("Plik README.md nie znaleziony")
    ...
    with tab4:
        ...
        tg = tg.rename(columns={"genre": "Gatunek", "tracks_count": "Liczba utworów"})
    ...
    st.title("Analiza utworów artysty (iTunes)")
    st.markdown("---")
    st.info("Zacznij od wpisania nazwy artysty w panelu bocznym i kliknięcia przycisku \"Pobierz i analizuj\" lub wyświetl dokumentację projektu, aby dowiedzieć się więcej.")
